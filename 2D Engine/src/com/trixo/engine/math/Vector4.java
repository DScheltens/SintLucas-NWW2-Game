...
import com.trixo.engine.utils.math.Platform;
...
public class Vector4 implements IVector4, Serializable {
    ...
    /**
     * Multiplies this vector by a scalar and stores the result back in this
     * vector.
     * 
     * @return a reference to this vector, for chaining.
     */
    ...
    /**
     * Multiplies this vector by a matrix (V * M) and stores the result back in
     * this vector.
     * 
     * @return a reference to this vector, for chaining.
     */
    ...
    public boolean epsilonEquals(IVector4 other, double epsilon) {
        return (Math.abs(x - other.x()) < epsilon && Math.abs(y - other.y()) < epsilon && Math.abs(z - other.z()) < epsilon && Math
                .abs(w - other.w()) < epsilon);
    }

    @Override
    // from interface IVector4
    ...
    public Vector4 mult(IMatrix4 matrix, Vector4 result) {
        ...
        return result.set(m00 * vx + m01 * vy + m02 * vz + m03 * vw, m10 * vx + m11 * vy + m12 * vz + m13 * vw, m20 * vx + m21
                * vy + m22 * vz + m23 * vw, m30 * vx + m31 * vy + m32 * vz + m33 * vw);
    }
